...
export default async function handler(req: any, res: any) {
  if (req.method !== 'POST') return json(res, 405, { error: 'method_not_allowed' })import { json, readBody } from '../_lib/http.js'
import { sb, logWebhook } from '../_lib/db.js'
import { validateWebhook } from '../_lib/kashier.js'
import { ensureIssued, syncCard } from '../_lib/issuance.js'
import { notify } from '../_lib/notify.js'
...
export default async function handler(req: any, res: any) {
  ...
  if (req.method === 'POST') {
    ...
    return json(res, 200, { ok: true, note: 'method ignored' })
  }

  const code = String(
    data.merchantOrderId || data.orderId || data.order_id || data.merchant_order_id || ''
  ).trim()
  const status = String(data.paymentStatus || data.status || '').toUpperCase()

  await logWebhook('kashier', `payment.${status || 'unknown'}`, data)

  try {
    const v = validateWebhook({ data })
    if (!v.ok) {
      await logWebhook('kashier', 'signature.note', { code, note: v.note }, true, v.note)
    }
    ...
    if (order.status === 'pending') {
      ...
      await notify('order_paid', {
        order_code: code, customer: order.customer_name,
        phone: order.phone, amount_egp: order.amount_egp, card_usd: order.card_usd
      })
